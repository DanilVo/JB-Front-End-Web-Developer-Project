...
function onToggleChange(element) {
  ...
  if (userCoins.length > 5) {
    ...
    const element1 = element.parentElement.parentElement.parentElement.id;
    const findCoinFromLS = coinsList.find((item) => item.id === element1);
    
    clearCoin(findCoinFromLS);
  }
}
...
function placeCoinsInModal(element) {
    ...
    const cards = document.querySelectorAll(
      "#selectedCoins > #responsiveLayout"
    );
  cards.forEach((card) => {
    const cardInput = card.querySelector("input");
    cardInput.checked = true;
    cardInput.setAttribute("oninput", `replaceCoin(this,element)`); //maybe to remove this function and pass straight 'element'
...
function htmlTemplate(data) {
  // const cards = document.querySelector(placement);
  let html = "";
